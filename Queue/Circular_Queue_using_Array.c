...
struct Queue *Enqueue(struct Queue *q1)
{
    ...
    scanf("%d", &(q1->arr[(++q1->r) % q1->size]));
    return q1;
}

struct Queue *Dequeue(struct Queue *q1)
{
    printf("%d is Dequeued succesfully.", q1->arr[(++q1->f) % q1->size]);
    return q1;
}

void peek(struct Queue *q1)
{
    printf("\nThe front most element in the queue is: %d", q1->arr[(q1->f + 1) % q1->size]);
    return;
}
